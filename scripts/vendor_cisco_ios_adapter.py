...
"""
Cisco IOS -> vendor-independent normalized state adapter.
...
Configuration state is independently cross-checked against
operational BGP summary state before normalized state is returned.
"""
...
class CiscoIosAdapterError(Exception):
    pass


def _require_text(value, field):
    if not isinstance(value, str):
        raise CiscoIosAdapterError(
...
def parse_cisco_ios_running_config(text):
    text = _require_text(
        text,
        "running_config",
    )
...
def parse_cisco_ios_bgp_summary(text):
    text = _require_text(
        text,
        "bgp_summary",
    )
...
def normalize_cisco_ios_state(
    ...
    *,
    ...
    config = parse_cisco_ios_running_config(
        running_config
    )

    summary = parse_cisco_ios_bgp_summary(
        bgp_summary
    )
